...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Mint tokens using the simple mintTo function
    const amountToMint = ethers.parseUnits(amount.toString(), 18); // Convert to 18 decimals
    
    console.log(`Minting ${amount} ARC tokens to ${playerAddress}`);
    
    // Execute the mint transaction
    const tx = await tokenContract.mintTo(playerAddress, amountToMint);
    await tx.wait();
